...
Path schema (Phase 9b, 2026-04-24 — OWC relocated hm4→hmj evening 2026-04-23): **Vault = `~/repos/Vault/` git clone** (GitHub `copper0722/vault`, system drive). **OWC Express 1M2 HDD physically attached to hmj**; hmj local path `/Volumes/OWC Express 1M2/VaultBinary/` = binary canonical (`_sidecar/`, `_archive/`, `_data/`, `_inbox/`, `_attachments/`, paired PDFs, SQLite, PNGs). Aliased as **`~/VaultBinary`** on every Mac. Dropbox role (unchanged): (a) `VaultBinary_Backup/` passive client-sync of OWC (1/3 backup dest + offline-hmj fallback); (b) `_inbox/` intake §7.5; (c) `BoAn/` clinic shared folder. Active backup pushes (NAS + B2) via `admin/backup/backup.sh`. **Cross-device runtime (SMB over Tailscale)**: **hmj** exposes whole OWC volume as SMB share `OWC%20Express%201M2`; **hm4/cm1/cmp** mount the volume at `~/VaultBinary-mnt/` via LaunchAgent (`com.copper.vaultbinary-mount.plist`); sshd needs Full Disk Access (TCC) on each client. Details Vault §8.12 + runbook `admin/runbooks/smb-hm4-vaultbinary.md` (pending rewrite→hmj). Iron rule §1.2 OWC carries binaries only; no `.md/.py/.sh/.json/.tsv/.txt/.yaml/.yml/.toml/.html/.css/.js`. Bridging symlinks (device-scoped): (1) `~/.claude → ~/repos/Vault/.claude/` — universal; (2) `~/repos/Vault/proj/note/_sidecar → ~/VaultBinary/_sidecar/` — gitignored; Obsidian-running devices only (cm1/cmp); hm4/hmj headless, no symlink; (3) `~/VaultBinary` → `~/VaultBinary-mnt/VaultBinary` on hm4/cm1/cmp (LaunchAgent mounts OWC volume at `~/VaultBinary-mnt`, symlink reaches subfolder `VaultBinary`); hmj direct: `~/VaultBinary` → `/Volumes/OWC Express 1M2/VaultBinary`.
