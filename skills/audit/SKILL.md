...
For Codex-Claude closed-loop incidents specifically, verify: independent Codex launchd (no Claude token dependency), mutually exclusive Claude-vs-Codex task selectors, `llm_handover_review`/repair-loop semantic tasks owned by Codex, `codex_closed_loop_health` script-lane sentinel, and a later Codex review after Claude repair before resolving the finding. Current policy: Codex outside audit runs once daily on hmj at 08:17 TST with `CODEX_AUDIT_FORCE=1` and posts a Slack DM report to Copper after every run (success, no due task, skip, or failure).
