...
(Earlier v4 path `medwiki/note/fuse/...` retired with the 2026-05-05 `wiki_raw`/`~/VaultBinary` split; v3 path `~/repos/note/fuse/...` retired with `~/repos/note/` shell removal 2026-05-03; v2 path `proj/wiki/fuse/...` retired with proj/ wrapper retire 2026-04-25; v1 root `_fuse/` retired 2026-04-20.)
...
```bash
mv _inbox/{file}.md ~/repos/_admin-private/workflow/fuse/{topic}_{YYYYMMDD}/source_{llm}.md
```
...
- `~/repos/_admin-private/workflow/fuse/{topic}_{YYYYMMDD}/fused_draft.md` — main consensus document (NOT yet in wiki/)
- `~/repos/_admin-private/workflow/fuse/{topic}_{YYYYMMDD}/_inventory.md` — Phase 1 output
- `~/repos/_admin-private/workflow/fuse/{topic}_{YYYYMMDD}/_alignment.md` — Phase 2 output
...
2. **Frontmatter harmonization**:
   ...
   - `parent: /_admin-private/workflow/fuse/{topic}_{YYYYMMDD}/fused_draft.md` (reverse pointer)
...
- CC retroactively creates `~/repos/_admin-private/workflow/fuse/{topic}_{YYYYMMDD}/` folder, materializes prompt.md (extract from Copper's memory if possible; else flag as "retro-import"), moves source files, proceeds to Step 3
...
Output: `~/repos/_admin-private/workflow/fuse/{topic}_{YYYYMMDD}/_inventory.md`. Show to Copper before proceeding.
...
Output: `~/repos/_admin-private/workflow/fuse/{topic}_{YYYYMMDD}/_alignment.md`. Show to Copper. Copper may resolve `[split]` items before Phase 3.

### Phase 3: Synthesis

Write `~/repos/_admin-private/workflow/fuse/{topic}_{YYYYMMDD}/fused_draft.md`. Rules:
...
Write session metadata to `~/repos/_admin-private/workflow/fuse/{topic}_{YYYYMMDD}/_index.md` status trail. DO NOT move `fused_draft.md` to wiki yet — Step 4 handles that.

## Folder layout (reminder)

```
~/repos/_admin-private/workflow/fuse/
