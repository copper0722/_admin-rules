...
publish: false              # boolean opt-in gate. NEVER true unless Copper sets it.
                            # downstream publishing pipeline gates on publish:true; agent-share
                            # canonical (gdrive, hm4/mbp-mounted) replaces the prior textbook-share-sync
                            # script lane (retired 2026-05-17 with the agent-share migration).
...
## Long-source dispatch policy (≥ 1500-line proofread.md)

For any chapter whose `proofread.md` exceeds ~1500 lines, **do not dispatch
a single agent to produce both `note-public.md` + `note-private.md` in one
run**. The agent stream watchdog (600s no-progress timeout) consistently
fires at the public→private transition, leaving the private note unwritten
and the agent killed mid-stream. Confirmed on Washington Manual 38e ch10
Pulmonary (2152L) and ch14 Treatment of Infectious Diseases (2617L) — both
agents wrote public successfully then stalled before completing private.

**Required dispatch pattern for long sources** (revised 2026-05-15):

| `proofread.md` size | dispatch pattern |
|---|---|
| ≤ 1500 lines | single agent, both notes in one run (default) |
| 1500-2000 lines | two agents in series: public-only, then private-only |
| ≥ 2000 lines | split private into ≥3 sub-section agents OR defer private |

The 600s stream watchdog fires reliably when a single Write tool call
emits a 1000+ line payload against a 2000+ line source. Even a focused
private-only agent on a 2152-line source stalled mid-write 2026-05-15
(confirmed on Washington Manual 38e ch10). For very-large chapters the
private content must either be written in batched Edit-append calls by
multiple agents, or deferred until a streaming-friendly dispatch
mechanism is available.

All long-source agents must carry `note_version: "medical-note-writer-v2.7"`
pin in the prompt explicitly (skill spec mandates it but agents often
drop the key).

Recovery from a stalled long-source agent: salvage whatever was written
(usually the public note completed before the stall), then either accept
public-only coverage or dispatch sub-section private agents.

## Section bleed in full-book OCR (cross-cutting risk)

When `proofread.md` is generated by splitting a full-book OCR dump using
in-body anchor scan (i.e. no PDF bookmark or page-mapping ground truth),
the per-section file content frequently bleeds into adjacent sections.
Verified in Pocket Medicine 9e: at least 9 of 13 section files contain
30-400 lines of foreign-section content. The frontmatter `title:` may say
"Critical Care" while the body is appendix formulas, or "GI" while a third
of the body is nephrology.

**Sub-agent obligation**: before applying the user's stated topic
breakdown, scan the actual `proofread.md` body and verify the chapter
covers what the filename claims. Faithfully render what is present;
explicitly flag any section-bleed gap in the agent's completion report.
Do not fabricate missing content to match the filename.
